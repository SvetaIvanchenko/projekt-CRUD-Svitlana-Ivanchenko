//EncjaB.js Dziyana Porakhonka (rozszerzenie modulu Svitlany Ivanchenko)
    ...
    <form method="post" action="${editItem ? `/update/${editItem.id}` : "/add"}">
      <div class="fields">
        ...
        <div class="field">
          ...
          <input name="username" placeholder="Twoje imię" value="${editItem ? escapeHtml(editItem.username ?? "") : ""}" />
        </div>
        <div class="field">
          <label>Data wpisu</label>
          <input name="review_date" type="date" value="${editItem?.review_date ?? ""}" />
        </div>
      </div>
...
app.post("/add", (req, res) => {
    ...
    db.prepare(`
    INSERT INTO reviews (title, year, genre, kind, rating, username, review_date, review)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
